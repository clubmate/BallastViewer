...
struct KeywordCountsTests {
    /// PEOPLE(1) > ANNA(2), PEOPLE > BOB(3), PLACES(4) > BERLIN(5)
    ...
    @Test func countsRollUpSubtreesAndDeduplicatePerPhoto() {
        ...
        #expect(counts[1] == 3)  // photos 10, 11, 12
        #expect(counts[2] == 2)  // photos 10, 11
        #expect(counts[3] == 1)  // photo 11
        #expect(counts[4] == 1)  // photo 13
        #expect(counts[5] == 1)  // photo 13
    }
    ...
    @Test func staleAssignmentIdsAreSkipped() {
        ...
        #expect(counts[99] == nil)
        #expect(counts[2] == 1)
        #expect(counts[1] == 1)
    }

    @Test func emptyInputsYieldEmptyCounts() {
        #expect(KeywordCounts.compute(keywordIdsByPhoto: [:], tree: tree).isEmpty)
