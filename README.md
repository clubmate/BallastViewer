...
- **AI auto-tagging with a local vision-language model:** set up a *keyword questionnaire* — a list of questions about a photo, each with fixed answers that map to your keywords ("How many people are the subject? none / one / two / three / group", "What is the gender of the main person? female / male / mixed / none", angle, age, lighting…) — and BallastViewer asks a vision-language model (Qwen3.5 or another Hugging Face MLX model of your choice, downloaded once, running entirely on your Mac) every question about every photo. Judgement follows the main subject, so a passer-by in the background does not make a portrait "mixed". Answers arrive as dashed pending chips in the inspector: accept ✓ to make the keyword real (then it flows into the file like any other), reject ✗ and that photo is never suggested that keyword again. Right-click a smart collection (or ALL PHOTOS) and choose Auto-Tag Photos to scan exactly that set; a REVIEW KEYWORDS row appears in the sidebar while suggestions are waiting, so you can walk the queue photo by photo. Right-click that row to discard every pending suggestion at once — the emergency exit after a run you did not like: nothing is remembered as rejected, and it can be undone. Questionnaires live in the library and can be switched on and off per run (one for portraits, another for street scenes), replies are cached so re-runs and re-mapped answers cost no time, and a newer model is a text field away (any MLX-converted vision model by its Hugging Face id). The system prompt is yours to edit, and two switches trade time for care: let the model think before it answers, and send photos at full resolution. Before a long run, Photo ▸ Preview Auto-Tagging on Selection shows the model's answers for a few selected photos without assigning anything, so you can tune the wording first; an answer can end the questionnaire ("no person" makes gender or age moot). Model downloads show speed and time left, and an interrupted download continues where it stopped. Apple Silicon only. Nothing touches your files until you accept.
