...
- **AI keyword suggestions:** describe a visual keyword in plain English ("someone talking on a phone") and BallastViewer suggests it on matching photos — entirely on-device via Apple's MobileCLIP, nothing leaves your Mac. Suggestions arrive as dashed pending chips in the inspector: accept ✓ to make the keyword real (then it flows into the file like any other), reject ✗ and that photo is never suggested that keyword again. Right-click a smart collection (or ALL PHOTOS) and choose Auto-Tag Photos to scan exactly that set; a REVIEW KEYWORDS row appears in the sidebar while suggestions are waiting, so you can walk the queue photo by photo. Right-click that row to discard every pending suggestion at once — the emergency exit after a run you did not like: nothing is remembered as rejected, and it can be undone. A keyword that covers several unrelated looks takes multiple prompts separated by | — the best match counts. Matches are judged by how much better your description fits than a plain "a photo", so one sensitivity setting works for broad prompts ("a woman") and specific ones ("a red vintage car") alike; describe what the whole picture shows ("a group of men", not just "a man" for team photos). Optionally the system learns as you go (Settings ▸ AI ▸ Learn from confirmed photos, off by default): every confirmed photo of a keyword becomes an example, and with enough examples the suggestions rely more on what your accepted photos actually look like than on the words you typed. With learning on, rejections count too: a photo that looks more like something you rejected than like anything you accepted is held back, so one ✗ on a burst frame silences its siblings. Leave it off for fully predictable, prompt-only scoring. Nothing touches your files until you accept, and keywords without a description are never suggested.
