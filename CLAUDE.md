...
- **U49 / MLX:** the app links `mlx-swift-lm` (on-device vision-language models). MLX compiles Metal shaders, so (1) the app builds **only through `xcodebuild`** (`swift build` cannot compile Metal — BallastCore stays MLX-free so `swift test` keeps working), (2) Xcode 26 needs the separate Metal toolchain once: `DEVELOPER_DIR=/Applications/Xcode.app xcodebuild -downloadComponent MetalToolchain`, (3) the app is **arm64-only** (`ARCHS: arm64` in project.yml), (4) `xcodebuild` needs `-skipPackagePluginValidation -skipMacroValidation` (mlx-swift's "CudaBuild" plugin; both scripts and release.yml pass them — without them a fresh Xcode fails with "Validate plug-in CudaBuild"). Models live in the shared Hugging Face cache `~/.cache/huggingface/hub`; headless VLM check: `BV_TEST=1 BV_TEST_OPEN=<lib> BV_TEST_VLM=<n> BV_TEST_VLM_MODEL=mlx-community/Qwen3.5-2B-4bit BV_TEST_QUIT=1` (the 2B model is the one downloaded on the dev Mac; add `BV_TEST_VLM_TREE=1` for the follow-up/open-question branch, `BV_TEST_VLM_MULTI=1` for a multiple-choice scene question, `BV_TEST_VLM_PROMPT=1` to print the prompt). Questionnaires are edited in the AI window (AI ▸ Keyword Questionnaires…, visible while Settings ▸ AI ▸ Enable AI is on). Free-question check: `BV_TEST=1 BV_TEST_OPEN=<lib> BV_TEST_VLM_ASK="<question>" BV_TEST_VLM_MODEL=… BV_TEST_QUIT=1`. Backup check without a server: `BV_TEST=1 BV_TEST_OPEN=<lib> BV_TEST_BACKUP=<dir> [BV_TEST_BACKUP_RSYNC=1] BV_TEST_QUIT=1` (rotates the anchor and writes it to the file first).
...
The app NEVER alters image data. Rotation is instant because the view rotates the stored orientation (Q5); keywords, ratings and (since U52) rotation are written into the image files automatically by the debounced write-through (`MetadataWriteThrough`, 2 s per photo, flushed on photo change, before a backup and at quit; `needsFileWrite` flag survives crashes). Only two code paths write image files — `MetadataWriter.write` (the automatic write-through: `xmp:Rating`, `dc:subject`, `lr:hierarchicalSubject`, EXIF orientation via a second byte-verbatim copy pass) and `MetadataWriter.writeOrientation` (BallastPicker rotation) — and both copy the compressed image data byte-for-byte (`CGImageDestinationCopyImageSource`), patching metadata only, via temp file + atomic replace. Backups (U53) COPY image files elsewhere, they never write into them. (A PNG without any metadata chunk gets the XMP packet spliced in as an `iTXt` chunk ahead of IDAT — ImageIO drops the patch otherwise; IDAT bytes are never touched.)
...
- `README.md` is the public GitHub face (repo is public). **When a user-facing feature lands, update the README's Features section in the same commit** — keep it prose-level (what the user gets), details stay in PLAN.md. No screenshots (user decision 2026-08-30: the demo library is not representative).
